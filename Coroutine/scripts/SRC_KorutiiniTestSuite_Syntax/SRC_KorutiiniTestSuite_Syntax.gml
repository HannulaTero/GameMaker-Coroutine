...
function KorutiiniTestSuite_Syntax()
{
  ...
  BEGIN 
    EXIT // To just not execute anything in practice.
    ...
    map     = ds_map_create();
    list    = ds_list_create();
    buffer  = buffer_create(32, buffer_fixed, 1);
    WHILE THEN END
    REPEAT THEN END
    FOREACH IN THEN END
    FOREACH IN [ ] THEN END
    FOREACH IN { } THEN END
    FOREACH IN map THEN END
    FOREACH IN list THEN END
    FOREACH IN buffer THEN END
